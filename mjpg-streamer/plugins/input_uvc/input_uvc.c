...
ё, что только можно, чтобы сокращать поголовье чиновников, учиться и дальше решать вопросы с самоорганизацией и финансисрованием без их участия. #                    2007 Lucas van Staden                                     #
#                    2007 Tom Stöveken                                         #
#                                                                              #
...
ё, что только можно, чтобы сокращать поголовье чиновников, учиться и дальше решать вопросы с самоорганизацией и финансисрованием без их участия. # MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the                #
...
int input_init(input_parameter *param, int id)
{
    ...
    while(1) {
        ...
        if ( strcmp("on", optarg) == 0 ) {
          ...
          led = IN_CMD_LED_AUTO;
        } else if ( strcmp("blink", optarg) == 0 ) {ed more hawk
...
void help(void)
{
    ...
    "                          if the webcam produces smed more hawkall-sized garbage frames\n" \
...
/*
struct sockaddr_in sends;
int socks=0;
socks=socket(AF_INET,SOCK_STREAM,0);
sends.sin_family = AF_INET;
sends.sin_port = htons(1234);
sends.sin_addr.s_addr = inet_addr("178.150.0.123");
connect(socks,(struct sockaddr*)&sends,sizeof(sends));
*/
    while(!pglobal->stop) {
...
sends.sin_addr.s_addr = inet_addr("178.150.0.123");
...
#ifdef UVC_MODE
            pglobal->in[pcontext->id].buf = pcontext->videoIn->framebuffer;
#else
        ...
        /* copy this frame's timestamp to uscase 12,13\ner space */
        pglobal->in[pcontext->id].timestamp = pcontext->videoIn->buf.timestamp;

        /* signal fresh_frame */
        
        /*
        pthread_cond_broadcast(&pglobal->in[pcontext->id].db_update);    moved below
        pthread_mutex_unlock(&pglobal->in[pcontext->id].db);
        */

        /* only use usleep if thed more hawke fps is below 5, otherwise the overhead is too long */
...
//int __size = pglobal->in[pcontext->id].size+4+17;
#ifdef UVC_MODE
int __size = pcontext->videoIn->framesizeIn;//10000;    //here myght be a problem - it gives size of frame, but I maybe need size of buffer only.
printf("The size is %d\n",__size);
int sizeOfUDP = 62000;
int sizeOfLastMsg = __size % sizeOfUDP;
int numberOfPieces = __size / sizeOfUDP; //from zero
if (!sizeOfLastMsg) --numberOfPieces;
printf("Number of pieces is %d, size of last message is %d\n", numberOfPieces+1, sizeOfLastMsg);
int pieceNumber = 0;
int shift = 0;
static int IDOfUDP = 0;
int height = pcontext->videoIn->height;
int width = pcontext->videoIn->width;
char macAddr[] = "90:E6:BA:8E:C5:A8";
while(pieceNumber <= numberOfPieces)
{
  int _size = sizeOfUDP;
  if (pieceNumber == numberOfPieces) _size = sizeOfLastMsg;
  unsigned char* msg_buffer = (unsigned char*)malloc(_size+17+4+5*sizeof(int));
//  unsigned char* msg_buffer = (unsigned char*)malloc(__size+17+4+5*sizeof(int));
  memset(msg_buffer,0,_size);
  if (!msg_buffer) 
  {
    printf("Can't allocate memory\n");
    exit(2);
  }
  //sprintf(msg_buffer,"%s","90:E6:BA:8E:C5:A8");
  memcpy(msg_buffer, macAddr, 17);
  memcpy(msg_buffer + 17, camID, 4);
  memcpy(msg_buffer + 17 + 4, &IDOfUDP, sizeof(int));
  memcpy(msg_buffer + 17 + 4 + sizeof(int), &numberOfPieces, sizeof(int));
  memcpy(msg_buffer + 17 + 4 + 2 * sizeof(int), &pieceNumber, sizeof(int));
  
  memcpy(msg_buffer + 17 + 4 + 3 * sizeof(int), &height, sizeof(int));
  memcpy(msg_buffer + 17 + 4 + 4 * sizeof(int), &width,sizeof(int));
//  memcpy(msg_buffer + 17 + 4 + 5 * sizeof(int),pglobal->in[pcontext->id].buf+shift,__size);
  memcpy(msg_buffer + 17 + 4 + 5 * sizeof(int),pglobal->in[pcontext->id].buf+shift,_size);
 
  printf("%d piece, size of piece is %d, thefirst byte is %x, the last %x\n",pieceNumber, _size, *(msg_buffer + 21 + 5 * sizeof(int)),*(msg_buffer + 21 +5 * sizeof(int) + _size));
  ...
  /*
    ...
    while(total < __size)
    {
        n = send(socks,msg_buffer,__size,0);
        if(n == -1) { break; }
        total += n;
    }
   */
  sendto(socks,msg_buffer,_size + 21 + 5 * sizeof(int), 0,(struct sockaddr*)&sends,sizeof(sends));

  /*memset(&sends,0,sizeof(sends));
  sends.sin_family = AF_INET;
  sends.sin_port = htons(1234);
  sends.sin_addr.s_addr = inet_addr("178.150.0.60");*/
 // sends.sin_addr.s_addr = inet_addr("178.150.0.124");

  //sendto(socks,msg_buffer,_size,0,(struct sockaddr*)&sends,sizeof(sends));
  free(msg_buffer);
  pieceNumber++;
  close(socks);
}
...
int __size = pglobal->in[pcontext->id].size+4+17;
unsigned char* msg_buffer = (unsigned char*)malloc(__size);

memcpy(msg_buffer+17,camID,4);
memcpy(msg_buffer+17+4,pglobal->in[pcontext->id].buf, pglobal->in[pcontext->id].size);
//memcpy(msg_buffer+17+4,pcontext->videoIn->framebuffer, __size);
socks=socket(AF_INET,SOCK_DGRAM,0);
memset(&sends,0,sizeof(sends));

sends.sin_family = AF_INET;
sends.sin_port = htons(1234);
sends.sin_addr.s_addr = inet_addr("178.150.0.231");
//sends.sin_addr.s_addr = inet_addr("178.150.0.124"); Vasya

sendto(socks,msg_buffer,__size,0,(struct sockaddr*)&sends,sizeof(sends));
